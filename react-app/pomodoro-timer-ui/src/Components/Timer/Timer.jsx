...
export default function Timer() {
  ...
  function playSound() {
    if (
      settingsStates.notifSound === typewriter ||
      settingsStates.notifSound === "typewriter"
    ) {
      playTypewriter();
    }
    if (
      settingsStates.notifSound === achievement ||
      settingsStates.notifSound === "achievement"
    ) {
      playAchievement();
    }
    if (
      settingsStates.notifSound === standard ||
      settingsStates.notifSound === "standard"
    ) {
      playStandard();
    }
    if (
      settingsStates.notifSound === bikebell ||
      settingsStates.notifSound === "bike"
    ) {
      playBikebell();
    }
    if (
      settingsStates.notifSound === dingdong ||
      settingsStates.notifSound === "doorbell"
    ) {
      playDingdog();
    }
  }
    ...
    setTimeout(async () => {
      ...
      if (settingsStates.session == shortBreak) {
        ...
        if (authStates.loggedIn) {
          const { data, error } = await apiClient.addSession({
            session_type: "PomoZone",
            duration: pomozoneTime,
          });
          if (error) {
          ...
          <div className="buttons">
            ...
            >
              <img
                ...
                alt={isRunning ? "pause timer" : "start timer"}
              ></img>
            </button>
            <button
              className={`${
